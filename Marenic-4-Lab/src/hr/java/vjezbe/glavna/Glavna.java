...
public class Glavna {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < ObrazovnaUstanova.MIN_INPUT; i++) {
				System.out.printf("Unesite podatke za obrazovnu ustanovu (%s/%s): %n", i+1, ObrazovnaUstanova.MIN_INPUT);
				...
				ispisProfesoraPoMjestu(profesori);
				
				List<Predmet> predmeti = new ArrayList<Predmet>();
				Map<Profesor, List<Predmet>> profesoriSaPredmetima = new HashMap<Profesor, List<Predmet>>();
				Map<Predmet, List<Profesor>> predmetiSaProfesorima = new HashMap<Predmet, List<Profesor>>(); 
				for (int j = 0; j < Predmet.MIN_INPUT; j++) {
					...
					// zadatak 3
					if (!predmetiSaProfesorima.containsKey(predmet)) {
						predmetiSaProfesorima.put(predmet, new ArrayList<Profesor>());
					}					
					predmetiSaProfesorima.get(predmet).add(predmet.getNositelj());	
				}				
				
				ispisProfesoraSaPredmetima(profesoriSaPredmetima);
				ispisPredmetimasaProfesorima(predmetiSaProfesorima);
	...
	private static Profesor unosProfesora(Scanner input, Integer i, Integer ukupnoUnosa) {
		...
		System.out.print("Naziv mjesta: ");
		String nazivMjesta = input.nextLine();
		
		return new Profesor(sifra, ime, prezime, titula, nazivMjesta);		 
	}
	...
	private static void ispisProfesora(List<Profesor> profesori) {
	...
	private static void ispisProfesoraPoMjestu(List<Profesor> profesori) {
	...
	private static void ispisPredmetimasaProfesorima(Map<Predmet, List<Profesor>> predmetiSaProfesorima) {
		System.out.println();
		for (Predmet predmet : predmetiSaProfesorima.keySet()) {
			System.out.printf("Predmet %s predaju slijedeći predmeti:%n", predmet.getNaziv());
			Integer count = 1;
			for (Profesor profesor : predmetiSaProfesorima.get(predmet)) {
				System.out.printf("%s. %s %s%n", count, profesor.getPrezime(), profesor.getIme());
				count++;
			}
		}		
	}
