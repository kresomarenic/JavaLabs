...
public class StudentUnosController {
	...
	public void unesi() {
		...
		if (!errors.isEmpty()) {
			...
			try {
				List<Student> pronadjeniStudenti = BazaPodataka.dohvatiStudentePremaKriterijima(new Student(null, null, null, txtJmbag.getText(), null));
				if (!pronadjeniStudenti.isEmpty()) {
					Student student = pronadjeniStudenti.get(0);
					ButtonType btnSpremi = new ButtonType("Spremi", ButtonData.OK_DONE);
					ButtonType btnOdustani = new ButtonType("Odustani", ButtonData.CANCEL_CLOSE);
					Alert alert = new Alert(AlertType.INFORMATION, "", btnSpremi, btnOdustani);
					alert.setHeaderText("Postoji student sa ovim JMBAG-om.");
					alert.setTitle("Postojeći student");		
					alert.setContentText("Pritiskom na gumb \"Spremi\" postojeći student će biti ažuriran.");		
					Optional<ButtonType> result = alert.showAndWait();
					
					if (result.get() == btnSpremi) {
						studenti.remove(studenti.filtered(s -> s.getJmbag().toLowerCase().contains(txtJmbag.getText().toLowerCase())).stream().collect(Collectors.toList()).get(0));
						...
						BazaPodataka.azurirajStudenta(student);
