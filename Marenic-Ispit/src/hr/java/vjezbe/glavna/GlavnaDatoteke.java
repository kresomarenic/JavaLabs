...
public class GlavnaDatoteke {
	...
	private static List<KomisijskiIspit> komisijskiIspiti;
	...
	public static void main(String[] args) {
		...
		try (Scanner input = new Scanner(System.in)) {
			...
			komisijskiIspiti = ucitajKomisijskeIspite(KomisijskiIspit.INPUT_FILE_NAME, KomisijskiIspit.NBR_OF_LINES_PER_RECORD);
...
//			ispisObrazovnihUstanova();
	...
	private static void ispisKomisijskihIspita() {
		
		System.out.println("\n-----------------\nISPIS KOMISIJSKIH ISPITA:");
		
		Map<Student, Integer> studentiIIzlazci = new HashMap<Student, Integer>();		
		
		for (KomisijskiIspit ispit : komisijskiIspiti) {
			Long brojIzlazaka = ispiti.stream().filter(i -> i.getStudent() == ispit.getStudent() && i.getPredmet() == ispit.getPredmet()).count();
			
			if (studentiIIzlazci.get(ispit.getStudent()) == null) {
				studentiIIzlazci.put(ispit.getStudent(), 1);
			} else {
				studentiIIzlazci.put(ispit.getStudent(), studentiIIzlazci.get(ispit.getStudent()) + 1);
			}
			
			System.out.printf("Predmet: %s -- Student: %s %s -- Broj standardnih izlaza: %s \n", ispit.getPredmet().getNaziv(), ispit.getStudent().getPrezime(), ispit.getStudent().getIme(), brojIzlazaka);
		}	
		
		System.out.println("\n-----------------\nSTUDENT KOJI JE NAJVIŠE PUTA IZAŠAO NA KOMISIJSKI ISPIT:");
		
		Integer maxIzlazaka = studentiIIzlazci.entrySet().stream().mapToInt(e -> e.getValue()).max().getAsInt();
		
		List<Student> studentiSNajviseIzlazaka = studentiIIzlazci.entrySet().stream()
				.filter(e -> e.getValue().equals(maxIzlazaka)).map(e -> e.getKey()).collect(Collectors.toList());
		
		studentiSNajviseIzlazaka.stream().forEach(s -> System.out.printf("%s %s", s.getPrezime(), s.getIme()));
	}
	...
	private static List<KomisijskiIspit> ucitajKomisijskeIspite(String inputFileName, Integer nbrOfLinesPerRecord) {
		
		List<KomisijskiIspit> ispiti = new ArrayList<>();
		List<String> inputData = Collections.emptyList();
		Path inputFile = Path.of(DATA_LOCATION + inputFileName);
		try {			
			inputData = Files.readAllLines(inputFile);
			for (int i = 0; i < inputData.size(); i += nbrOfLinesPerRecord) {
				...
				List<String> idClanova = Arrays.asList(inputData.get(i + 5).split(" "));
				Profesor clan1 = profesori.stream().filter(p -> p.getId() == Long.parseLong(idClanova.get(0))).findAny().orElse(null);
				Profesor clan2 = profesori.stream().filter(p -> p.getId() == Long.parseLong(idClanova.get(1))).findAny().orElse(null);
				Profesor clan3 = profesori.stream().filter(p -> p.getId() == Long.parseLong(idClanova.get(2))).findAny().orElse(null);
				
				ispiti.add(new KomisijskiIspit(id, predmet, student, ocjena, datumIspita, clan1, clan2, clan3));				
			}
		} catch (IOException e) {
			System.out.println("Došlo je do pogreške kod čitanja datoteke");
			log.error("Greška u čitanju datoteke!", e);
